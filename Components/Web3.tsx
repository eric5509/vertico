...
export default function Web3() {
  ...
  return (
    <div className="bg-black my-16 text-white relative cursor  overflow-hidden py-16 text-center">
      <div className="absolute h-12 w-12 rounded-full bg-amber-500"></div>
      <p className="text-2xl font-bold flex-wrap text-center items-center justify-center w-full flex gap-2">
        {"We work with the leading Web3 projects like".split(" ").map((data, key) => (
          <motion.span variants={var1} viewport={{once: true}} custom={key} initial="initial" whileInView="animate" className="inline-block">{data}{' '}</motion.span>
        ))}
      </p>
